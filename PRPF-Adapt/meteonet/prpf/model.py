...
def copy_shared_weights(source, target):
    ...
    copied = {name: value for name, value in source_state.items() if name in target_state and target_state[name].shape == value.shape}
...
class ReliabilityRouter(nn.Module):
    def __init__(self, radar_dim, pangu_dim, state_dim=64):
        super().__init__()
        self.pangu_mapper = nn.Conv2d(pangu_dim, radar_dim, 1, bias=False)
        self.state_encoder = nn.Sequential(
            nn.Conv2d(radar_dim + 2, state_dim, 3, padding=1),
            nn.GroupNorm(_gn_groups(state_dim), state_dim),
            nn.SiLU()
        )
        self.hidden = nn.Sequential(
            nn.Conv2d(radar_dim + state_dim + 1, radar_dim, 3, padding=1),
            nn.GroupNorm(_gn_groups(radar_dim), radar_dim),
            nn.SiLU()
        )
        self.weight_head = nn.Conv2d(radar_dim, 2, 1)
        self.conflict_head = nn.Conv2d(radar_dim, 1, 1)
        self.delta = nn.Conv2d(radar_dim, radar_dim, 1)
        nn.init.zeros_(self.weight_head.weight)
        nn.init.zeros_(self.weight_head.bias)
        nn.init.zeros_(self.delta.weight)
        nn.init.zeros_(self.delta.bias)

    def forward(self, radar_feat, pangu_feat, x_obs):
        height, width = radar_feat.shape[-2:]
        pangu = self.pangu_mapper(F.interpolate(pangu_feat, size=(height, width), mode="bilinear", align_corners=False))
        last = F.interpolate(x_obs[:, -1], size=(height, width), mode="bilinear", align_corners=False)
        change = F.interpolate(x_obs[:, -1] - x_obs[:, -2], size=(height, width), mode="bilinear", align_corners=False)
        state = self.state_encoder(torch.cat([radar_feat, last, change], dim=1))
        radar_motion = torch.cat([F.pad(change[:, :, :, 1:] - change[:, :, :, :-1], (0, 1)), F.pad(change[:, :, 1:, :] - change[:, :, :-1, :], (0, 0, 0, 1))], dim=1)
        pangu_motion = pangu[:, :2]
        motion = (F.normalize(radar_motion, dim=1, eps=1e-6) * F.normalize(pangu_motion, dim=1, eps=1e-6)).sum(dim=1, keepdim=True)
        hidden = self.hidden(torch.cat([pangu, state, motion], dim=1))
        weights = torch.softmax(self.weight_head(hidden), dim=1)
        conflict = torch.sigmoid(self.conflict_head(hidden))
        routed = radar_feat + weights[:, 1:2] * (1.0 - conflict) * self.delta(pangu)
        return routed, {"radar_weight": weights[:, 0:1], "pangu_weight": weights[:, 1:2], "conflict_weight": conflict}

class PanguFiLMCalibrator(nn.Module):
    def __init__(self, feature_dim, pangu_dim, hidden_dim=64, scale_limit=0.1, shift_limit=0.1, echo_threshold=1.41, echo_temperature=0.25):
        super().__init__()
        self.scale_limit = float(scale_limit)
        self.shift_limit = float(shift_limit)
        self.echo_threshold = float(echo_threshold)
        self.echo_temperature = float(echo_temperature)
        self.pangu_mapper = nn.Conv2d(pangu_dim, hidden_dim, 1)
        self.state_mapper = nn.Conv2d(2, hidden_dim, 3, padding=1)
        self.modulation = nn.Sequential(
            nn.SiLU(),
            nn.Conv2d(hidden_dim * 2, hidden_dim, 3, padding=1),
            nn.SiLU(),
            nn.Conv2d(hidden_dim, feature_dim * 2, 1)
        )
        nn.init.zeros_(self.modulation[-1].weight)
        nn.init.zeros_(self.modulation[-1].bias)

    def forward(self, feature, pangu_feat, x_obs):
        height, width = feature.shape[-2:]
        pangu = F.interpolate(pangu_feat, size=(height, width), mode="bilinear", align_corners=False)
        pangu = self.pangu_mapper(pangu)
        last = F.interpolate(x_obs[:, -1], size=(height, width), mode="bilinear", align_corners=False)
        change = F.interpolate(x_obs[:, -1] - x_obs[:, -2], size=(height, width), mode="bilinear", align_corners=False)
        state = self.state_mapper(torch.cat([last, change], dim=1))
        scale, shift = self.modulation(torch.cat([pangu, state], dim=1)).chunk(2, dim=1)
        confidence = torch.sigmoid((last - self.echo_threshold) / self.echo_temperature)
        scale = self.scale_limit * torch.tanh(scale) * confidence
        shift = self.shift_limit * torch.tanh(shift) * confidence
        self.regularizer = scale.square().mean() + shift.square().mean()
...
class PRPF_SetGoGAN_Generator(nn.Module):
    def __init__(self,
        ...
        if self.model_variant == "m3":
            self.reliability_router = PanguFiLMCalibrator(
                att_dim, Cw, echo_threshold=router_echo_threshold
            )
    ...
    def forward(self, x_obs, x_wfm):
        ...
        for t in range(self.Tout):
            ...
            if self.use_crossattn:
                ...
                F1 = radar_feat
            if self.model_variant == "m3":
                F0 = self.reliability_router(F0, p_t, x_obs)
                router_penalties.append(self.reliability_router.regularizer)
                F1 = self.reliability_router(F1, p_t, x_obs)
                router_penalties.append(self.reliability_router.regularizer)
        ...
        self.router_regularizer = torch.stack(router_penalties).mean() if router_penalties else pred_radar.new_zeros(())
